...
path2save_json = '/home/atuin/b211dd/b211dd19/data/dataset/tinyllava/eval/text_files_small_dataset/flickr30k_test.json'
...
prefix_path_img = "flickr30k/test/"

TEMPLATE = {
    "id": "{img_id}",
    "image": prefix_path_img + "{filename}",
    "conversations": [
        {
            "from": "human",
...
for index, row in tqdm(df.iterrows()):
    ...
    if split == 'test':
        shutil.copy2(os.path.join(flickr30k_images_path, filename),
                     os.path.join(dir2save_img, prefix_path_img+filename))


with open(path2save_json, 'w') as outfile:
    json.dump(test_list, outfile, indent=4)

print(f"The size of testing set is {len(test_list)}!")
