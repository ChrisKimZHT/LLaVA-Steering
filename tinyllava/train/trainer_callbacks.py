...
from tinyllava.model.mores_model_shell import MoReSModelShell
...
class SaveCallback(transformers.TrainerCallback):
    ...
    def on_train_end(self,
                        ...
                        control: transformers.TrainerControl,
                        **kwargs):
        log('saving the end checkpoint ...')
        self.trainer.args.output_dir = os.path.join(self.trainer.args.output_dir, "end")
        self.training_recipe.save(self.trainer.model, self.trainer)
        self.trainer.args.output_dir = os.path.dirname(self.trainer.args.output_dir)
...
class WandbLogCallback(transformers.TrainerCallback):
    ...
    def log_model_size_into_wandb(self):
        ...
        if isinstance(model.language_model, MoReSModelShell):
            total_params_llm_without_mores = sum(p.numel() for p in model.language_model.model.parameters())
            table.add_data("total_params_llm_without_mores", total_params_llm_without_mores)
            table.add_data("total_params_mores", total_params_llm - total_params_llm_without_mores)
