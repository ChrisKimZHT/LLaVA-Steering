...
from ..utils import log, init_mores_pos_configs
...
class TinyLlavaForConditionalGeneration(TinyLlavaPreTrainedModel):
    ...
    def forward(
        ...
        if isinstance(self.language_model, MoReSModelShell):
    ...
    def parse_mores_config(self, mores_config, model):
        for model_type, config_ in mores_config.items():
    ...
    def add_mores_adaptor_from_training_arguments(self, training_arguments):
        if not training_arguments.mores_config_path and not hasattr(self, 'mores_config'):
            log("No mores_config is provided, no mores adaptors will be added!")
        # load mores_config and add mores modules to model
        if training_arguments.mores_config_path and not hasattr(self, 'mores_config'):
            self.generate_mores_config(training_arguments.mores_config_path)

        if not hasattr(self, 'mores_pos_configs'):
            self.generate_mores_pos_configs(self.mores_config, self, training_arguments) if training_arguments.mores_pos_configs == None else None
        
        self.add_mores_adaptor()

    def load_mores_adaptor_from_ckpt(self, pretrained_model_path: str):
        self._extend_type_to_module_mapping()
        mores_save_path = os.path.join(pretrained_model_path, 'mores')
        if "language_model" in os.listdir(mores_save_path):
            lm_subpath = os.path.join(mores_save_path, 'language_model')
            self.language_model = MoReSModelShell.load(
                lm_subpath,
                self.language_model,
                model_register_name=full_name_to_LLM_register_name(self.config.llm_model_name_or_path),
                model_task_type='llm'
            )
            if "mores_pos_configs.json" in os.listdir(lm_subpath):
                with open(os.path.join(lm_subpath, "mores_pos_configs.json"), 'r') as file:
                    data = json.load(file)
                    self.mores_pos_configs = data
            # for possibly saving mores_config again later
            if "intervention_config.json" in os.listdir(lm_subpath):
                with open(os.path.join(lm_subpath, "intervention_config.json"), 'r') as file:
                    data = json.load(file)
                    self.mores_config = {'llm': data}


    def add_mores_adaptor(self):
        self._extend_type_to_module_mapping()
        mores_config = copy.deepcopy(self.mores_config)
        # llm mores adaptor
        if 'llm' in mores_config.keys():
            log("Adding MoReS adapters to llm...")
            mores_config_llm = MoReSConfigGenerator(**mores_config['llm'])
            self.language_model = get_mores_shell(self.language_model,
                                                mores_config_llm,
    ...
    def generate_mores_pos_configs(self, mores_config, model, training_arguments):
        ...
        if training_arguments.mores_share_weights is not None:
            configs2upate['mores_share_weights'] = training_arguments.mores_share_weights
        if training_arguments.intervene_modality is not None:
            configs2upate['intervene_modality'] = training_arguments.intervene_modality

        mores_pos_configs = init_mores_pos_configs()
        mores_pos_configs.update(configs2upate)

        if training_arguments.intervention_positions_2 is not None \
            and training_arguments.mores_share_weights_2 is not None \
            and training_arguments.intervene_modality_2 is not None: 
            log("There're a second mores_pos_configs to be loaded.")
            mores_pos_configs_2 = copy.deepcopy(mores_pos_configs)
            mores_pos_configs_2['intervention_positions'] = training_arguments.intervention_positions_2
            mores_pos_configs_2['mores_share_weights'] = training_arguments.mores_share_weights_2
            mores_pos_configs_2['intervene_modality'] = training_arguments.intervene_modality_2

            mores_pos_configs = [mores_pos_configs, mores_pos_configs_2]   # mores_pos_configs: Dict --> List[Dict]

        training_arguments.mores_pos_configs = mores_pos_configs
        self.mores_pos_configs = mores_pos_configs

    def generate_mores_config(self, mores_config_path):
        with open(mores_config_path, 'r') as file:
            self.mores_config = json.load(file)
        self.parse_mores_config(self.mores_config, self)
    ...
    @classmethod
    def from_pretrained(cls, repo_id: str, filename="pytorch_model.bin", revision="main"):
        raise NotImplementedError