...
@register_vision_tower('molecule_stm')
class MoleculeSTM(VisionTower):
    def __init__(self, cfg):
        super().__init__(cfg)
        molecule_node_model = GNN(
            num_layer=cfg.gin_num_layers,
            emb_dim=cfg.gin_hidden_dim,
            JK='last',  # default to 'last'
            drop_ratio=cfg.graph_drop_ratio,
            gnn_type='gin',  # default to 'gin'
        )

        self.molecule_stm = GNN_graphpred(
            emb_dim=cfg.gin_hidden_dim,
    ...
    def forward(self, mol, **kwargs):
        graph_features = self.molecule_stm.encode_mol(mol, proj=False, return_node_feats=False, eval=not self.training)
