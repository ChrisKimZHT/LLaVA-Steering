...
from .mores_model_utils import MoReSModelProxy, MoReSModelProxy4Generate
from .mores_mappings import *
...
def get_mores_shell(model, mores_config, model_register_name, model_task_type, set_device=True, disable_model_grads=True):
    """
    Create an instance of MoReS model.
    """
    assert not isinstance(model, MoReSModelShell), \
        "The model has been already integrated with MoReS method. The language_model/vision_tower can only be added with MoReS once!"
    # mores_model = MoReSModelShell(mores_config, model, MoReSModelProxy(model_register_name), model_task_type)
    mores_model = MoReSModelShell(mores_config, model, model_register_name, model_task_type)
    if set_device:
        mores_model.set_device(model.device)
    if disable_model_grads:
        mores_model.disable_model_gradients()    
    return mores_model

class MoReSModelShell(ReftModel):
    """
    Shell for MoReSModel, mainly forward() modified
    """
    def __init__(self, config, model, model_register_name, model_task_type, **kwargs):
        self.moresModelProxy = MoReSModelProxy(model_register_name)
        self.moresModelProxy4Generate = MoReSModelProxy4Generate(model_register_name)
    ...
    def load_intervenable_model(load_directory, model, local_directory=None, from_huggingface_hub=False):
        ...
        if "representations" in intervention_config:
            for mores_args in intervention_config['representations']:
                if "intervention" in mores_args:
                    inter_type = mores_args["intervention"]["type"]
                    del mores_args["intervention"]["type"]
                    ...
                    mores_args["intervention"] = INTERVENTION_TYPE_MAPPING[inter_type](**mores_args["intervention"])
                new_repres.append(mores_args)
    ...
    @staticmethod
    def load_mores_model(*args, **kwargs):
        model = MoReSModelShell.load_intervenable_model(*args, **kwargs)
    ...
    def load(*args, **kwargs):
        ...
        model = MoReSModelShell.load_mores_model(*args, **kwargs)
        return MoReSModelShell._convert_to_mores_model_shell(model,
                                                             model_register_name,
                                                             model_task_type)

    @staticmethod
    def _convert_to_mores_model_shell(mores_model,
                                      model_register_name,
                                      model_task_type):
        mores_model_shell = MoReSModelShell(mores_model.config, mores_model.model, model_register_name, model_task_type)
        # Copy any other necessary attributes
        for attr in vars(mores_model):
            setattr(mores_model, attr, getattr(mores_model, attr))
        return mores_model_shell
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        assert len(args) in [0, 1], "To much positional arguments for MoReSModelShell.forward()!"
        ...
        subspaces_ = kwargs.get("len_two_pos_configs", None)
        subspaces_ = subspaces_.tolist() if isinstance(subspaces_, torch.Tensor) else subspaces_
        ...
        if sources_ is None and activations_sources is None \
            ...
            if self.model_task_type == 'vision_tower':
                return self.model(self.moresModelProxy.get_adapted_args(base_, kwargs)[0],
                              **self.moresModelProxy.get_adapted_args(base_, kwargs)[1]), None
            elif self.model_task_type == 'llm':
                return self.model(**self.moresModelProxy.get_adapted_args(base_, kwargs)[1]), None
            else:
                raise ValueError('MoReSModel only applies to llm or vision_tower')
        ...
        if output_original_output_:
            ...
            if self.model_task_type == 'vision_tower':
                base_outputs = self.model(self.moresModelProxy.get_adapted_args(base_, kwargs)[0],
                                        **self.moresModelProxy.get_adapted_args(base_, kwargs)[1])
            elif self.model_task_type == 'llm':
                base_outputs = self.model(**self.moresModelProxy.get_adapted_args(base_, kwargs)[1])
            else:
                raise ValueError('MoReSModel only applies to llm or vision_tower')
        try:
            # intervene
            ...
            if self.model_task_type == 'vision_tower':
                counterfactual_outputs = self.model(self.moresModelProxy.get_adapted_args(base_, **kwargs)[0],
                                                    **self.moresModelProxy.get_adapted_args(base_, **kwargs)[1],
                                                    **model_kwargs)
            elif self.model_task_type == 'llm':
                counterfactual_outputs = self.model(**self.moresModelProxy.get_adapted_args(base_, **kwargs)[1],
                                                    **model_kwargs)
            else:
                raise ValueError('MoReSModel only applies to llm or vision_tower')
        ...
        **kwargs
        ...
        if output_original_output_:
            if self.model_task_type == 'llm':
                base_outputs = self.model.generate(**self.moresModelProxy4Generate.get_adapted_args(base_, kwargs)[1])
            else:
                raise ValueError('MoReSModel only applies to llm or vision_tower')
        ...
        try:
            ...
            if self.model_task_type == 'llm':
                counterfactual_outputs = self.model.generate(**self.moresModelProxy4Generate.get_adapted_args(base_, **kwargs)[1])
            else:
                raise ValueError('MoReSModel only applies to llm')
    ...
    def _broadcast_subspaces(
        ...
        if isinstance(subspaces, int):
            ...
            _subspaces = [[subspaces]*batch_size]*len(self.interventions)
        elif isinstance(subspaces, list) and isinstance(subspaces[0], list) and isinstance(subspaces[0][0], int):
            _subspaces = [subspaces]*len(self.interventions)
        else:
    ...
    def _prepare_inputs(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        """
        During training, there's _prepare_inputs in the trainer where one can make adjustment for mores model, while during inference, there's no such api.
...
class MoReSConfigGenerator(pyreft.ReftConfig):
    """
    generate mores config for mores methods.
    """
    def __init__(
        self, **kwargs,
    ):
        if "representations" in kwargs:
            new_repres = []
            for mores_args in kwargs['representations']:
                if "intervention" in mores_args:
                    inter_type = mores_args["intervention"]["type"]
                    del mores_args["intervention"]["type"]
                    assert inter_type in INTERVENTION_TYPE_MAPPING, f'The intervention type doesn\'t exist, please use the following intervention types: [{", ".join(INTERVENTION_TYPE_MAPPING.keys())}]'
