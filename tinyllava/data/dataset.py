...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        self.text_preprocess = TextPreprocessMoReS(tokenizer, data_args.conv_version, data_args.mores_pos_configs) \
            if hasattr(data_args, 'mores_pos_configs') else TextPreprocess(tokenizer, data_args.conv_version)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'len_two_pos_configs' in instances[0] and all(ins.get("len_two_pos_configs") is not None for ins in instances):
            len_two_pos_configs = [instance['len_two_pos_configs'] for instance in instances]
