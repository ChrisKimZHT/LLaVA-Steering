...
def eval_model(args):
    ...
    data = json.load(open(os.path.join(args.eval_dir, 'HallusionBench.json'), 'r'))
    img_dir = os.path.join(args.eval_dir, 'hallusion_bench')

    answers_file = os.path.expanduser(args.answers_file)
    os.makedirs(os.path.dirname(answers_file), exist_ok=True)
    model.to(device='cuda')
    for i, line in enumerate(tqdm(data)):
        visual_input = line['visual_input']   # 0,1,2
