...
class LazyEvalDataset(Dataset):
    ...
    def __init__(self, list_data_dict: List[Dict],
        ...
        self.text_preprocess = TextPreprocessMoReS(tokenizer, eval_args.conv_mode, eval_args.mores_pos_configs) \
            if hasattr(eval_args, 'include_mores') else TextPreprocess(tokenizer, eval_args.conv_mode)
...
def eval_model(args):
    ...
    model.load_mores_adaptor_from_ckpt(args.pretrained_model_path) if args.include_mores else None

    converter = Converter2LlamaFormat(args.question_file)
    # questions = [json.loads(q) for q in open(os.path.expanduser(args.question_file), "r")]
    # questions = get_chunk(questions, args.num_chunks, args.chunk_idx)
    args.mores_pos_configs = model.mores_pos_configs if args.include_mores == True else None
    ...
    for data_dict in tqdm(data_loader):
        ...
        intervention_locations = data_dict['intervention_locations'] if hasattr(args, 'include_mores') and args.include_mores == True else None
        # image preprocessing
...
if __name__ == "__main__":
    ...
    parser.add_argument("--include_mores", type=bool, default=False)
