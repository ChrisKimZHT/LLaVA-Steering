...
from tinyllava.eval.halbench_utils import *
...
if __name__ == "__main__":
    ...
    save_json_path_vd = os.path.join(args.eval_dir, 'hallusion_output_vd_model.json')
    save_json_path_vs = os.path.join(args.eval_dir, 'hallusion_output_vs_model.json')
    os.makedirs(args.eval_dir, exist_ok=True)

    data_vd = []
    data_vs = []
    with open(args.answers_file) as json_file:
    ...
    data_vd = evaluate_by_chatgpt(data_vd, model_output_entry, model_correctness_entry, args.api_key, load_json=load_json, save_json_path=save_json_path_vd)
    data_vd = check_same_by_chatgpt(data_vd, model_output_entry, args.api_key, load_json=load_json, save_json_path=save_json_path_vd)
    #time.sleep(60) #
    try:
        data_vs = evaluate_by_chatgpt(data_vs, model_output_entry, model_correctness_entry, args.api_key, load_json=load_json, save_json_path=save_json_path_vs)
        data_vs = check_same_by_chatgpt(data_vs, model_output_entry, args.api_key, load_json=load_json, save_json_path=save_json_path_vs)
    except:
        time.sleep(60)
        data_vs = evaluate_by_chatgpt(data_vs, model_output_entry, model_correctness_entry, args.api_key, load_json=load_json, save_json_path=save_json_path_vs)
        data_vs = check_same_by_chatgpt(data_vs, model_output_entry, args.api_key, load_json=load_json, save_json_path=save_json_path_vs)
