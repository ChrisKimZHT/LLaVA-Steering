...
class LoRATrainingRecipe(BaseTrainingRecipe):
    ...
    def training_model_converse(self, model):
        ...
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(model, self.lora_skip_module),   # 应用 LoRA 适配的模块列表 通常是线性层或其他需要调整的层 
            lora_dropout=self.training_arguments.lora_dropout,
            bias=self.training_arguments.lora_bias,   # 是否在 LoRA 模块中包含偏置项
            task_type="CAUSAL_LM",
        )
