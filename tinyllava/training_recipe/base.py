...
class BaseTrainingRecipe:
    ...
    def _llm_tune_type_setting(self, model):
        ...
        assert tune_type in LLM_TUNE_TYPES, f'tune_type {tune_type} not supported in this training recipe!'
        if tune_type == 'full':
        ...
        if tune_type == 'mores':
            self.support_gradient_checkpoint(model.language_model.model, self.training_arguments.gradient_checkpointing)
        else:
    ...
    def _vision_tower_tune_type_setting(self, model):
        ...
        assert tune_type in VT_TUNE_TYPES, f'tune_type {tune_type} not supported in this training recipe!'
        if tune_type == 'full':
    ...
    def _connector_tune_type_setting(self, model):
        ...
        assert tune_type in CONNECTOR_TUNE_TYPES, f'tune_type {tune_type} not supported in this training recipe!'   
        if tune_type == 'full':
    ...
    def save(self, model, trainer):
        ...
        if 'finetune' in self.training_arguments.output_dir and self.training_arguments.pretrained_model_path is not None: # for finetune stage
            if trainer.deepspeed:   # 当前的训练过程是否使用 DeepSpeed 进行的
                torch.cuda.synchronize()   # 确保所有的 CUDA 操作在继续之前都已完成
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:   # 当使用分布式数据并行 (DDP) 或其他分布式训练技术时 每个进程都会有一个唯一的 local_rank
    ...
    def load_base_modules_from_model_args(self,model, model_args):
    ...
    def load(self, model, model_args={}):
        self.load_base_modules_from_model_args(model, model_args)
        if 'adapter' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json')): 
            from ..utils.peft.src.peft import PeftModel
            print('Loading adapter-Peftmodel...')
            model.language_model = PeftModel.from_pretrained(model.language_model, self.training_arguments.pretrained_model_path)
        elif 'lora' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json')):
            from peft import PeftModel
            print('Loading LoRA weights...')
            model = PeftModel.from_pretrained(model, self.training_arguments.pretrained_model_path)
            print('Merging LoRA weights...')
            model = model.merge_and_unload()
        elif 'ia3' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json')):
            from peft import PeftModel
            print('Loading IA3-Peftmodel...')
            model.language_model = PeftModel.from_pretrained(model.language_model, self.training_arguments.pretrained_model_path)
        elif 'oft' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json')):
            from peft import PeftModel
            print('Loading OFT-Peftmodel...')
            model.language_model = PeftModel.from_pretrained(model.language_model, self.training_arguments.pretrained_model_path)
