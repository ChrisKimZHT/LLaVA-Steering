...
from ..model.mores_mappings import *
from ..model.mores_model_shell import *


@register_training_recipe('mores')
class MoReSTrainingRecipe(BaseTrainingRecipe):
    def __init__(self, training_arguments):
        super().__init__(training_arguments)
        self.training_arguments = training_arguments
        self.mores_skip_module = ['connector', 'vision_tower', 'language_model']
    ...
    def save(self, model, trainer):
        ...
        if isinstance(model.language_model, MoReSModelShell):
            lm = model.language_model.model
            if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
                mores_lm_output_dir = os.path.join(os.path.join(self.training_arguments.output_dir, 'mores'), 'language_model')
                model.language_model.save(mores_lm_output_dir)   # save mores params
                with open(os.path.join(mores_lm_output_dir, 'intervention_config.json'), 'w') as json_file:
                    json.dump(model.mores_config['llm'], json_file, indent=4)
                if model.mores_pos_configs:
                    with open(os.path.join(mores_lm_output_dir, 'mores_pos_configs.json'), 'w') as json_file:
                        json.dump(model.mores_pos_configs, json_file, indent=4)
                print(f"The mores parameters of language model has been successfully saved in {mores_lm_output_dir}.")
        else:
